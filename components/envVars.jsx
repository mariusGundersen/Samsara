...
export default ({config, url}) => (
  <section data-viewmodel="spirit/EnvVarsVM" data-model={JSON.stringify(config)} className="box">
    <h3>
      <span>
        <Icon of="tag" />
        Environment variables
      </span>
      <button className="pure-button button-xsmall button-minimal button-no-padding" data-bind="click:add" title="Add environment variable"><Icon of="plus" /></button>
    </h3>
    <div data-bind="foreach: envVars" className="box-list">
    ...
    <div data-bind="ifnot:envVars().length" className="box-list">
      <div className="box-form box-list-item">
        <p className="text-center text-muted">Click the pluss icon above to add a new environment variable</p>
      </div>
    </div>
