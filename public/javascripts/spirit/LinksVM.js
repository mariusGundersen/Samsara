define(['spirit/EditLink', 'knockout', 'deco/qvc'], function(EditLink, ko, qvc){
  return function LinksVM(model, when){
    ...
    this.links = ko.observableArray(toLinksList(model.links).map(function(link){
  ...
  function toLinksList(links){
    var result = [];
    if(links){
      for(var alias in links){
        if(links.hasOwnProperty(alias)){
          result.push({
            alias: alias, 
            spirit: typeof(links[alias]) == 'string' ? links[alias] : links[alias].spirit || ''
          });
        }
      }
    }
