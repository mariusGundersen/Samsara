...
import ContainersMenu from "./menu.js";
...
router.get("/:id", async function (req, res, next) {
  ...
  const logs = await streamToString(
    container.prettyLogs(true, { stdout: true, stderr: true, tail: 50 })
  );
...
router.get("/:id/logs/download", async function (req, res, next) {
  ...
  res.setHeader(
    "Content-disposition",
    "attachment;filename=" + config.Name.substr(1) + ".log"
  );
  const logs = await container.prettyLogs(false, {
    stdout: true,
    stderr: true,
  });
  logs.pipe(res);
