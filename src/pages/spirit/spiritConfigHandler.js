...
export default [
  qvc.command(
    "setSpiritImageAndTag",
    async function (command) {
      let containerConfig = await samsara().spirit(command.name)
        .containerConfig;
  ...
  qvc.command(
    ...
    {
      key: new NotEmpty(
        "Please specify a key for the new environment variable"
      ),
    }
  ),
  ...
  qvc.command("removeEnvVar", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.environment = containerConfig.environment.filter(
      (env) => env.key !== command.key
    );
    await containerConfig.save();
  }),
  qvc.command(
    ...
    {
      containerPath: new NotEmpty(
        "Please specify a containerPath for the new volume"
      ),
    }
  ),
  ...
  qvc.command("removeVolume", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.volumes = containerConfig.volumes.filter(
      (volume) => volume.containerPath !== command.containerPath
    );
    await containerConfig.save();
  }),
  qvc.command(
    ...
    },
    {
      ...
      hostIp: new Pattern(
        ...
        "The host ip must follow the pattern #.#.#.#"
      ),
    }
  ),
  qvc.command(
  ...
  qvc.command("removePort", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.ports = containerConfig.ports.filter(
      (port) => port.hostPort !== command.hostPort
    );
    await containerConfig.save();
  }),
  qvc.command(
  ...
  qvc.command("removeLink", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.links = containerConfig.links.filter(
      (link) => link.alias !== command.alias
    );
    await containerConfig.save();
  }),
  qvc.command(
  ...
  qvc.command("setVolumesFrom", async function (command) {
    ...
    volumesFrom
      .filter(
        (volumeFrom) =>
          volumeFrom.spirit === command.oldFromSpirit &&
          volumeFrom.container === command.oldFromContainer
      )
      .forEach((volumeFrom) => {
  ...
  qvc.command("removeVolumesFrom", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.volumesFrom = containerConfig.volumesFrom.filter(
      (volumeFrom) =>
        volumeFrom.spirit !== command.fromSpirit &&
        volumeFrom.container !== command.fromContainer
    );
  ...
  qvc.command("removeLabel", async function (command) {
    let containerConfig = await samsara().spirit(command.name).containerConfig;
    containerConfig.labels = containerConfig.labels.filter(
      (label) => label.key !== command.key
    );
    await containerConfig.save();
  }),
