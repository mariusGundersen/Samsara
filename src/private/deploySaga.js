...
export default function deploySaga(eventBus) {
  return new EventSaga(eventBus, (saga) => {
    saga.createOn("spirit.deploy.start", function (data) {
      this.data = {
        name: data.spirit,
        life: data.life,
        isDeploying: true,
    ...
    saga.on("deployStatusRequest", function (data) {
      eventBus.emit("spiritDeployStatus", {
        target: data.target,
        data: this.data,
      });
    ...
    saga.on("spirit.deploy.stage", function (data) {
      var activeIndex = this.data.plan
        .map((step) => step.state)
        .indexOf("pending");
    ...
    saga.on("spirit.deploy.message", function (data) {
      if (data.message) {
          ...
          if ("progressDetail" in data.message) {
            eventBus.emit("spiritDeployPullStatus", {
              ...
              data: {
                ...
                progressDetail: data.message.progressDetail,
              },
            });
    ...
    saga.on("spirit.deploy.stop", function (data) {
